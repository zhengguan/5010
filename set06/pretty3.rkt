...
(define (expr-to-string e width)
  (if (fit-in-one-line? e width)
      (expr-to-one-string e)
      (expr-to-string-with-blanks e 0 width)))
...
;; expr-to-one-string : Expr -> String
...
;; expr-to-string-with-blanks 
;; : Expr NonNegInt NonNegInt -> ListOfString
;; GIVEN: an expression, the number of blanks blank-num, a width.
;; RETURNS: A representation of the expression as a sequence of lines,
;; with each line represented as a string of length not greater than 
...
(begin-for-test
  ...
  (check-equal?
   ...
   (list "* 1 2")))
;; STRATEGY: Cases on e : Expr
(define (expr-to-string-with-blanks e blanks-num width)
  (cond
    [(integer? e) (integer-expr-to-string e blanks-num width)]
    [(sum-exp? e) (sum-expr-to-string e blanks-num width)]
    [(mult-exp? e) (mult-expr-to-string e blanks-num width)]))
