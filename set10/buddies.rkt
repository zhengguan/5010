...
(define World%
 ...
 #;   (define/public (on-tick)
    ...
    ;; -> Target%
    (define/public (for-test:get-target)
      target)
    
    ;; -> ToyFactory%
    (define/public (for-test:get-toy-factory)
      toy-factory)
...
(define SquareToy%
  (class* object% (StatefulToy<%>)
    ...
    (field [mode "outline"] ; Mode
           ...
           [half-length (/ length 2)]); Half side length
    ...
    (define/public (drag x-pos y-pos)
      ...
      (local 
        ...
        (for-each 
         (lambda(elt) (send elt select-by-buddy x-pos y-pos))
         new-buddies)
        ; add this to buddies list of each new buddy.
        (for-each 
         (lambda(elt) (send elt add-buddy this))
         new-buddies)
        ; add new buddies to buddies list.
    ...
    ;; -> Integer
    (define/public (for-test:toy-x-dist)
      x-dist)
    
    (define/public (for-test:toy-y-dist)
      y-dist)
    
    ;; -> ListOf<SquareToy%>
    (define/public (for-test:toy-buddies)
      buddies)
...
(define ToyFactory%
  (class* object% ()
    ...
    ;; -> Integer
    (define/public (for-test:target-x-pos)
      target-x-pos)
    
    (define/public (for-test:target-y-pos)
      target-y-pos)
...
(define (square-toy-equal? st1 st2)
  (and
   ...
   (= (send st1 toy-y)
      (send st2 toy-y))
   (= (send st1 for-test:toy-x-dist)
      (send st2 for-test:toy-x-dist))
   (= (send st1 for-test:toy-y-dist)
      (send st2 for-test:toy-y-dist))
   (equal? (send st1 toy-selected?)
           (send st1 toy-selected?))
  ...
  (local
    ...
    (and
     (= (send sw1 get-speed)
        (send sw2 get-speed))
     (target-equal?
      (first(send sw1 get-target))
      (first(send sw2 get-target)))
...
(define (toy-factory-equal? tf1 tf2)
  (and
   (= (send tf1 for-test:target-x-pos)
      (send tf2 for-test:target-x-pos))
   (= (send tf1 for-test:target-y-pos)
      (send tf2 for-test:target-y-pos))))
...
(begin-for-test
  (local
    ((define the-world (new World%))
     (define the-target (send the-world for-test:get-target))
     (define the-toy-factory 
       (send the-world for-test:get-toy-factory))
     (define the-toys (send the-world get-toys)))
    ; tests for key event
    (check-equal?
     (length the-toys)
     0)
    ; add a toy
    (send the-world on-key "s")
    (check-equal? 
     (length (send the-world get-toys))
     1))
  
  ; tests for target
  (local
    ((define the-world (new World%)))
    (send the-world on-mouse 201 251 "button-down")
    (check string=?
           (send (send the-world for-test:get-target) get-color)
           "orange")
    (check target-equal?
           (send the-world for-test:get-target)
           (new Target%
                [x 200]
                [y 250]
                [selected? true]
                ...
                [y-dist -1]))
    ; drag target
    (send the-world on-mouse 211 261 "drag")
    (check target-equal?
           (send the-world for-test:get-target)
           (new Target%
                [x 210]
                ...
                [x-dist -1]
                [y-dist -1]))
    ; check toy factory's target position
    (check-equal? 
     (send (send the-world for-test:get-toy-factory) 
           for-test:target-x-pos)
     210)
    (check-equal? 
     (send (send the-world for-test:get-toy-factory) 
           for-test:target-y-pos)
     260)
    (send the-world on-mouse 211 261 "button-up")
    (check string=?
           (send (send the-world for-test:get-target) get-color)
           "black")
  )
  
  ; tests for square toy
  (local
    ((define the-world (new World%)))
    (send the-world on-key "s")
    ;; select square toy
    (send the-world on-mouse 215 265 "button-down")
    (check string=?
           (send (first (send the-world get-toys)) toy-color)
           "red")
    (check square-toy-equal?
           (first (send the-world get-toys))
           (new SquareToy%
                [x 200]
                [y 250]
                [x-dist -15]
                ...
                [selected? true]))
    ; drag square toy
    (send the-world on-mouse 230 280 "drag")
    (check square-toy-equal?
           (first (send the-world get-toys))
           (new SquareToy%
                [x 215]
                ...
                [world the-world]
                [selected? true]))
    (send the-world on-mouse 230 280 "button-up")
    ; add another square toy
    (send the-world on-key "s")
    (check-equal? 
     (length (send the-world get-toys))
     2)
    ; select one square toy
    (send the-world on-mouse 185 235 "button-down")
    (check string=? 
           (send (first (send the-world get-toys)) toy-color)
           "red")
    (check string=? 
           (send (second (send the-world get-toys)) toy-color)
           "green")
    ; No dragging, no adding buddies.
    (check-equal?
     (length (send (first (send the-world get-toys)) 
                   for-test:toy-buddies))
     0)
    (send the-world on-mouse 185 235 "button-up")
    
    (send the-world on-mouse 230 280 "button-down")
    (check string=? 
           (send (first (send the-world get-toys)) toy-color)
           "green")
    (check string=? 
    ...
    ; No dragging, no adding buddies.
    (check-equal?
     (length (send (second (send the-world get-toys)) 
                   for-test:toy-buddies))
     0)
    (send the-world on-mouse 230 265 "drag")
    (check square-toy-equal?
           (second (send the-world get-toys))
           (new SquareToy%
                [x 215]
                [y 250]
                [x-dist -15]
    ...
    (check string=? 
           ...
           "red")
    (check equal?
           (send (first (send the-world get-toys)) toy-selected?)
           false)
    (check equal?
           (send (second (send the-world get-toys)) toy-selected?)
           true)
    
    ; button up
    (send the-world on-mouse 230 265 "button-up")
    (check-equal?
     (length (send (first (send the-world get-toys)) 
                   for-test:toy-buddies))
     1)
