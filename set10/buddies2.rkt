...
(define Target%
  (class* object% ()
    ...
    (define/public (target-x-dist)
      x-dist)
    
    (define/public (target-y-dist)
      y-dist)
...
(define SquareToy%
  (class* object% (StatefulToy<%>)
    ...
    ;; overlap-with-square-toy?-helper : Integer Integer -> Boolean
    ;; GIVEN: the center position of a square toy.
    ;; RETURNS: true iff this toy is overlapped with the square toy.
    (define/public (overlap-with-square-toy?-helper x-coor y-coor)
      (and (<= (abs (- x-coor x)) length)
           (<= (abs (- y-coor y)) length)))    
    ...
    ;; EFFECT: Set this toys's center.
    ;; DETAIL: this toy is selected or a buddy of the selected toy.
    (define/public (change-center x-pos y-pos)
      (set! x (+ x-pos x-dist))
      (set! y (+ y-pos y-dist)))
...
(define (square-toy-equal? st1 st2)
  (and
   ...
   (equal? (send st1 toy-selected?)
           (send st2 toy-selected?))
   (andmap
    (lambda(t1 t2) (square-toy-equal? t1 t2))
    (send st1 toy-buddies)
    (send st1 toy-buddies))))
...
(define (target-equal? t1 t2)
  (and
   ...
   (= (send t1 target-y)
      (send t2 target-y))
   (equal? (send t1 target-selected?)
           (send t2 target-selected?))
   (= (send t1 target-x-dist)
      (send t2 target-x-dist))
   (= (send t1 target-y-dist)
      (send t2 target-y-dist))
   (string=? 
    (send t1 target-color)
    (send t2 target-color))))
...
;; StetefulWorld% StetefulWorld% -> Boolean
;; GIVEN: two stateful worlds.
;; RETURNS: true iff the given two stateful worlds are equal
(define (stateful-world-equal? sw1 sw2)
  (and
   (target-equal? (send sw1 )
                  (send sw2 ))
   (andmap 
    square-toy-equal?
    (send sw1 )
