...
(define reorder-status3 (make-reorder 1 10))
(define htdp
  (make-book 
...
(define sicp2
  (make-book
   0070004846 "Structure and Interpretation of Computer Programs"
   "Harold Abelson" "McGraw-Hill Science/Engineering/Math" 3688
   2655 10 reorder-status3 0.09))
(define sicp3
  (make-book
   0070004846 "Structure and Interpretation of Computer Programs"
   "Harold Abelson" "McGraw-Hill Science/Engineering/Math" 3688
   2655 20 empty-reorder 0.09))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; reorder-present? : Reorder -> Boolean
...
(define (inventory-potential-profit inv)
  (foldr
   ...
   ; GIVEN: a book and the potential profit of the rest books.
   ; RETURNS: the potential profit including this book.
   (lambda(b p) (+ (book-potential-profit b) p))
   0
   inv))
...
(define (inventory-total-volume inv)
  (foldr
   ; Book Number -> Number   
   ; GIVEN: a book and the volume of the rest books.
   ; RETURNS: the volume of books including the given book.
   (lambda(b v) (+ (book-total-cuft b) v))
   0
   inv))
...
(define (price-for-line-item-helper inv isbn n)
  (foldr
   ...
   ; RETURNS: the given book's price multiply n if its ISBN equals to
   ; isbn, else returns the given maybe integer
   (lambda(b mi) (if(same-isbn? b isbn) (price-for-books b n) mi))
   false
   inv))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; fillable-now? : Inventory Order -> Boolean
...
(define (fillable-now? o inv)
  (andmap 
   ; LineItem -> Boolean
   ; RETURNS: true iff the given line item is fillable by inv now.
   (lambda(li) (line-item-fillable-now? inv li))
   o))
...
(define (line-item-fillable-now?-helper inv isbn n)
  (ormap
   ; Book -> Boolean
   ; RETURN: true iff the given book's ISBN equals isbn and its number
   ; is greater or equal to n
   (lambda(b) (and (same-isbn? b isbn) (book-fillable-now? b n)))
   inv))
...
#;(define (days-til-fillable  o inv)
    (cond
      ...
      [else (max-maybe-integer
             (days-til-line-item-fillable  inv (first o))
             (days-til-fillable (rest o) inv))]))

(define (days-til-fillable o inv)
  (foldr
   ; LineItem MaybeInteger-> MaybeInteger
   ; GIVEN: a line item and the days until the rest line items are 
   ; fillable
   ; RETRUNS: the number of days until the given line item is 
   ; fillable. Returns false if the line item is not fillable.
   (lambda(li mi) 
     (max-maybe-integer
      (days-til-line-item-fillable inv li)
      mi))
   0
   o))
...
;; days-til-line-item-fillable : Inventory LineItem -> MaybeInteger
...
;; EXAMPLES: in helper function's TESTS
;; STRATEGY: Structural Decomposition on li : LineItem
(define (days-til-line-item-fillable inv li)
...
;; days-til-line-item-fillable-helper : 
;;   Inventory Integer PosInt -> MaybeInteger
;; GIVEN: an inventory, the ISBN of a book, the required number of the
;; book.
...
;; EXAMPLES: in TESTS
;; STRATEGY: HOFC
#;(define (days-til-line-item-fillable-helper inv isbn n)
    (cond
      [(empty? inv) false]
      [else (if (same-isbn? (first inv) isbn)
...
(define (days-til-line-item-fillable-helper inv isbn n)
  (foldr
   ; Book MaybeInteger -> MaybeInteger
   ; RETURNS: the number of days until n copies of the given book is 
   ; fillable if the given book's ISBN equals to isbn. Otherwise 
   ; returns the given maybe integer
   (lambda(b mi) 
     (if(same-isbn? b isbn) 
        (days-til-book-fillable b n)
        mi))
   false
   inv))
...
(begin-for-test
  (check-equal? (days-til-line-item-fillable
                ...
                false)
  (check-equal? (days-til-line-item-fillable
                 inventory1 sicp-line-item-fillable) 
                0)
  (check-equal? (days-til-line-item-fillable
                 inventory1 sicp-line-item-unfillable) 
                false))
...
(define (price-for-order inv o)
  (foldr
   ...
   ; GIVEN: a line item and the price of the rest line items
   ; RETURNS: the price of line items including the given one
   (lambda(li price) (add-maybe-integer 
                      (price-for-line-item inv li)
                      price))
   0
   o))
...
(define (inventory-after-order inv o)
  (foldr
   ...
   ; RETURNS: a inventory like the given one, but with the given line
   ; item been filled.
   (lambda(li inv-before) (inventory-after-line-item inv-before li))
   inv
   o))
...
(define (increase-prices inv publisher percentage)
  (map
   ; Book -> Book
   ; RETURNS: the book's price increased by percentage if the book is
   ; published by publisher
   (lambda(b) 
     (if(same-publisher? b publisher) 
        (book-increase-price b percentage)
        b))
   inv))
...
;; all reorders that were due in 1 day are now avaliable, and all 
;; other reorders have their expected times decreased by 1.
;; STRATEGY: HOFC
(define (inventory-after-deliveries inv)
  (map
   book-after-deliveries
   inv))
...
;; book-after-deliveries : Book -> Book
;; RETURNS: a book representing the given book's tomorrow state, in 
;; which if its reorder state was bue in 1 day then the reordered 
;; copies are now avaliable, otherwise its expected times decreased by 1.
;; EXAMPLES: in TESTS
;; STRATEGY: Structural Decomposition on b : Book
(define (book-after-deliveries b)
  (make-book 
   (book-isbn b)
   (book-title b)
   (book-author b)
   ...
   (book-unit-price b)
   (book-unit-cost b)
   (copies-after-deliveries (book-copies b) (book-reorder b))
   (reorder-state-after-deliveries (book-reorder b))
   (book-cuft b)))

;; TESTS:
(begin-for-test
  (check-equal?
   (book-after-deliveries sicp2)
   sicp3))

;; copies-after-deliveries : Integer Reorder -> Integer
;; GIVEN: a book's copies and a reorder state.
;; RETURNS: the book's state after reorderd copies were avaliable.
;; EXAMPLES: in TESTS
;; STRATEGY: Structural Decomposition on r : Reorder
(define (copies-after-deliveries n r)
  (if (equal? (reorder-days r) 1)
      (+ (reorder-copies r) n)
      n))
...
(begin-for-test
  (check-equal?
   (copies-after-deliveries 10 reorder-status3) 20)
  (check-equal?
   (copies-after-deliveries 10 reorder-status1) 10))


;; reorder-state-after-deliveries : Reorder -> Reorder
;; GIVEN: a reorder status
;; RETURNS: the reorder status of next day
;; EXAMPLES: in TESTS
;; STRATEGY: Structural Decomposition on r : Reorder
(define (reorder-state-after-deliveries r)
  (if (equal? (reorder-days r) 1)
      (make-reorder 0  0)
      (make-reorder (- (reorder-days r) 1)
                    (reorder-copies r))))
...
(begin-for-test
  (check-equal?
   (reorder-state-after-deliveries reorder-status1)
   (make-reorder 6 10))
  (check-equal?
   (reorder-state-after-deliveries reorder-status3)
   (make-reorder 0 0)))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define reorder-status-1 (make-reorder 2 5))
(define book-1 
  (make-book 15 "How to Design Programs" 
             "Felleisen et al." "MIT Press"
             59 49 100 reorder-status-1 1/12))
