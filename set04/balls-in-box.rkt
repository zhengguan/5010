...
;; A Ball is a (make-ball Real Real Boolean String)
;; Interpretation:
...
(define paused-world-with-one-ball
  (make-world (cons unselected-ball-at-200-150 empty) 8 true 0 0 1))
...
(define (world-after-key-event w kev)
  (cond
    [(key=? kev "n") (world-add-ball w)]
    [(key=? kev " ") (world-paused?-toggled w)]
    [else w]))
...
(begin-for-test
  ...
  (check-equal? 
   ...
   (make-world empty 8 true 0 0 0))
  (check-equal? 
   (world-after-key-event (initial-world 8) "q")
   (initial-world 8)))
...
;; world-paused?-toggled : World -> World
;; RETURNS: a world like the given one, but with its paused? state 
;; toggled.
;; TESTS: see below TESTS
;; STRATEGY: Structural Decompositoin on w : World
(define (world-paused?-toggled w)
  (make-world
   (world-balls w)
   (world-ball-speed w)
   (paused?-toggled (world-paused? w))
   (world-x-coor w)
   (world-y-coor w)
   (world-n w)))   
...
(begin-for-test
  (check-equal?
   (world-paused?-toggled world-with-one-ball)
   (make-world (cons unselected-ball-at-200-150 empty) 8 true 0 0 1)))

;; paused?-toggled : Boolean -> Boolean
;; RETURNS: the toggled paused? state of the given one
;; EXAMPLES: see below TESTS
;; STRATEGY: Function Composition
(define (paused?-toggled b)
  (not b))
...
(begin-for-test
  (check-equal?
   (world-after-tick paused-world-with-one-ball)
   paused-world-with-one-ball)
  (check-equal?
...
(begin-for-test
  (check-equal?
   (balls-after-tick (list unselected-ball-at-200-150 
                           selected-ball-at-200-150
                           unselected-ball-at-400-150
                           (make-ball 400 150 false "left")) 8)
...
(define (ball-after-tick b speed)
  (if (ball-selected? b)
      b
      (ball-after-tick-helper b (ball-next-x-pos b speed))))


;; ball-next-x-pos : Ball PosInt -> PosInt
;; GIVEN: a ball and its speed(in pixels)
;; RETURNS: the given ball's x-pos in next tick if the canvas was
;; limitless.
;; EXAMPLES: see below TESTS
;; STRATEGY: Structural Decomposition on b : Ball
(define (ball-next-x-pos b s)
  (cond 
    [(string=? (ball-direction b) "right") (+ (ball-x-pos b) s)]
    [(string=? (ball-direction b) "left") (- (ball-x-pos b) s)]))
      
;; TESTS:
(begin-for-test
  (check-equal?
   (ball-next-x-pos unselected-ball-at-200-150 8)
   208)
  (check-equal?
   (ball-next-x-pos (make-ball 200 150 false "left") 8)
   192))

;; ball-after-tick-helper : Ball Real -> Ball
;; GIVEN: a ball and the x coordinate of its position in the next
;; tick if the canvas was limitless
;; RETURNS: the ball in next tick
;; TESTS: see below TESTS
;; STRATEGY: Structural Decomposition on b : Ball
(define (ball-after-tick-helper b x)
  (cond
    [(out-of-left-border? x )
     (make-ball BALL-LEFT-BORDER (ball-y-pos b) 
                (ball-selected? b) "right")]
    [(out-of-right-border? x)
     (make-ball BALL-RIGHT-BORDER (ball-y-pos b)
                (ball-selected? b) "left")]
    [else (make-ball x (ball-y-pos b)
                     (ball-selected? b) (ball-direction b))]))
...
(begin-for-test
  (check-equal?
   (ball-after-tick-helper (make-ball 0 150 false "left") -8)
   (make-ball BALL-LEFT-BORDER 150 false "right"))
  (check-equal?
   (ball-after-tick-helper (make-ball 400 150 false "right") 408)
   (make-ball BALL-RIGHT-BORDER 150 false "left")))


;; out-of-left-border? : Integer -> Boolean
;; out-of-right-border? : Integer -> Boolean
;; RETURNS: true iff the given integer is out of the specified border
;; EXAMPLES: see below TESTS 
;; STRATEGY: Function Compositoin
(define (out-of-left-border? x)
  (< x BALL-LEFT-BORDER))

(define (out-of-right-border? x)
  (> x BALL-RIGHT-BORDER))
...
(begin-for-test
  (check-equal?
   (out-of-left-border? 0) true)
  (check-equal? 
   (out-of-right-border? CANVAS-WIDTH) true))
