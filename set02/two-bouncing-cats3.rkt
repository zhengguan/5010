...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname two-bouncing-cats3) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
;; Two bouncing cat. (with anorhter version of world-after-key-event)
...
(define (world-after-key-event w kev)
  (make-world (cat-after-key-event (world-cat1 w) kev)
              (cat-after-key-event (world-cat2 w) kev)
              (paused?-state-after-key-event (world-paused? w) kev)))
...
;; cat-after-key-event : Cat KeyEvent -> Cat
;; GIVEN: a cat and a key event
;; RETURNS: the following cat
;; EXAMPLES:
;; STRATEGY: Structural Decomposition on c : Cat
(define (cat-after-key-event c kev)
  (if (cat-selected? c)
      (make-cat (cat-x-pos c) (cat-y-pos c) 
                (cat-selected? c) (key-to-direction (cat-direction c) kev))
      c))

;; key-to-direction :  Direction KeyEvent -> Direction
;; GIVEN: a key event and a cat's current direction
;; RETURNS: the cat's next direction
;; EXAMPLES:
;; STRATEGY: Cases on kev : KeyEvent
(define (key-to-direction d kev)
  (cond
    [(key=? kev "left") "west"]
    [(key=? kev "right") "east"]
    [(key=? kev "up") "north"]
...
;; pause-state-after-key-event : Boolean KeyEvent -> Boolean
;; GIVEN: a value describes the world's paused? state and 
;; a key event
;; RETURNS: the world's next paused? state
;; EXAMPLES:
;; STRATEGY: Cases on kev : KeyEvent
(define (paused?-state-after-key-event paused? kev)
  (if (key=? kev " ")
      (not paused?)
      paused?))
