...
;; one? : Int -> Boolean
;; GIVEN: an Integer 
;; RETURNS: true iff the Integer is 1
;; EXAMPLES:
;; (one? 2) = false
;; (one? 1) = true
(define (one? n)
...
;; STRATEGY: Cases on t
(define (color-at-next-second c t)
  (if (ready-to-change? t) (next-color c) c))
...
(define (ready-to-change? ts)
  (one? ts))

(check-equal? (ready-to-change? 1) true)
(check-equal? (ready-to-change? 17) false)
